...
int main() {
    ...
    printf("1. Конструктор по умолчанию:\n");
    matrix2d* m1 = matrix2d_create_default();
    matrix2d_print(m1);
    
    // 2. Конструктор с параметрами
    printf("\n2. Конструктор с параметрами (матрица 3x3):\n");
    ...
    printf("m2 < m3? %s\n", matrix2d_less(m2, m3) ? "да" : "нет");
    
    // 6. Получение строки и столбца
    printf("\n6. Получение строки и столбца:\n");
    int32_t* row = matrix2d_get_row(m2, 0);
    if (row) {
        printf("Строка 0: ");
    ...
    int32_t* col = matrix2d_get_col(m2, 0);
    if (col) {
        printf("Столбец 0: ");
        for (int i = 0; i < m2->size; i++) {
            printf("%d ", col[i]);
    ...
    if (m5) {
        ...
        if (m6) {
            printf("Обратная матрица (целочисленное приближение):\n");
            matrix2d_print(m6);
            matrix2d_free(m6);
        }
    }
